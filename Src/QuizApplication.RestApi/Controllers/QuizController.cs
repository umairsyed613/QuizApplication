...
using System.Linq;
using QuizRestApi.Model;
using QuizRestApi.Model.Domain;

namespace QuizRestApi.Controllers
{
    [Route("api/quizzes")]
    public class QuizController : Controller
    {
        ...
        public IEnumerable<QuizResponseModel> Get()
        {
            ...
            return quizzes.Select(quiz =>
                                      new QuizResponseModel { Id = quiz.Id, Title = quiz.Title });
        }
            ...
            return new QuizResponseModel
            {
                ...
                Questions = questions.Select(question => new QuizResponseModel.QuestionItem
                {
                    ...
                    Answers = answers.ContainsKey(question.Id)
                        ? answers[question.Id].Select(answer => new QuizResponseModel.AnswerItem { Id = answer.Id, Text = answer.Text })
                        : new QuizResponseModel.AnswerItem[0],
                    CorrectAnswerId = question.CorrectAnswerId
                }),
                Links = new Dictionary<string, string> { { "self", $"/api/quizzes/{id}" }, { "questions", $"/api/quizzes/{id}/questions" } }
            };
        }
